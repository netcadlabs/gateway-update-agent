...
{
    internal abstract class ConfigBase : IItem //**NDU-372
    {
        #region Model
        ...
        {
            //**NDU-372 return string.Concat(Helper.ReplaceInvalidFileNameChars(this.Type, "_"), "_custom.json");
            return Helper.ReplaceInvalidFileNameChars(this.ConfigFileName, "_");
        }
        ...
        {
            return Path.Combine(getDownloadDir(stt), string.Concat(this.Name, ".json"));
        }
        ...
        public IEnumerable<UpdateResult> DownloadIfRequired(IModule parent, ISettings stt, ILogger logger)
        {
            if (this.State == States.DownloadRequired)
            {
                ...
                logger.LogInformation($"Downloading {this.Name}...  Type:{this.URL}");
        ...
        protected abstract string yamlKey { get; }
        private void install(ISettings stt)
        {
            ...
            else
            {
                ...
                if (!File.Exists(configFn))
                    throw new Exception($"{configFn} file cannot be found! url:{this.URL}");
                ...
                this.YamlConnectorItems[this.yamlKey] = name;
            }
        }
